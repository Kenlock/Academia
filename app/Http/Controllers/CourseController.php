...
class CourseController extends Controller
{
    ...
    public function instructorCourseDelete($course_id = '',Request $request){
        $course_to_delete = Course::find($course_id);
        $course_to_delete->delete();
        $path = "course/" . $course_id;
        $directory_to_delete = Storage::deleteDirectory($path);
        return $this->return_output('flash', 'success', 'The course is now deleted', 'back', '200');
    ...
    public function courseView($course_slug = '', Request $request)
    {
        ...
        return view('site.course.view', compact('course', 'curriculum_sections', 'lectures_count', 'videos_count', 'video', 'course_breadcrumb', 'is_curriculum', 'count'));
    }
    ...
    public function readPDF($file_id)
    {
        ...
        if($file_details){
            ...
            $pdf_name = $file_details->file_name.'.'.$file_details->file_extension;
            $pdf_path = 'course/'.$file_details->course_id;

            return response()->file(storage_path('app/public/course/'.$file_details->course_id. '/'.$pdf_name));
        }
    }
    ...
    public function postLectureQuizDelete(Request $request){
        //delete from the coursefiles and course videos tables, then from the file system
        // 0 - video , 1- audio, 2- docs, 3 - text
        $lid = $request->input('lid');
        $course_id = $request->input('courseid');

        $media = \DB::table('curriculum_lectures_quiz')->where('lecture_quiz_id', $lid)->value('media_type');

        if($media == 1 || $media == 2){
          $deleteFile = CourseFiles::where('lecture_id' , $lid)->first();

          $filename = $deleteFile->file_name . "." . $deleteFile->file_type;

          unlink(storage_path('app\\public\\course\\'.$course_id.'\\'. $filename));
          $deleterow = CourseFiles::where('lecture_id' , $lid)->delete();
          $this->model->postLectureQuizDelete($request->input('lid'));
          echo "successfully deleted the audio";
        }elseif($media == 0){
          $deleteFile = CourseVideos::where('lecture_id' , $lid)->first();

          $filename = $deleteFile->video_title . "." . $deleteFile->video_type;

          unlink(storage_path('app\\public\\course\\'.$course_id.'\\'. $filename));
          $deleterow = CourseVideos::where('lecture_id' , $lid)->delete();
          $this->model->postLectureQuizDelete($request->input('lid'));
          echo "successfully deleted the video";
        }else{
          $deleterow = CourseFiles::where('lecture_id' , $lid)->delete();
          $this->model->postLectureQuizDelete($request->input('lid'));
        }
